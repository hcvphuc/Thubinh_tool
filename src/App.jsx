...
function App() {
    ...
    // Predefined Templates - Tết 2026 Edition
    const TEMPLATES = [
        // === TẾT - EM BÉ ===
        {
            id: 'baby_tet_lion',
            name: 'Múa Lân Tết',
            icon: '🦁',
            bgColor: 'linear-gradient(135deg, #d32f2f 0%, #ffd54f 100%)',
            category: 'baby',
            description: 'Bé cùng đội múa lân sư rộng, pháo hoa rực rỡ',
            prompt: 'Vietnamese Lunar New Year 2026 background with traditional lion dance performance, red and gold decorations, firecrackers, festive lanterns, joyful celebration atmosphere, Year of the Snake 2026, professional studio quality',
        ...
        },
        {
            ...
            category: 'baby',
            description: 'Bé mặc áo dài truyền thống với hoa mai vàng',
            prompt: 'Beautiful Vietnamese Tet 2026 background with yellow Mai flowers (apricot blossoms), traditional red decorations, soft golden lighting, elegant setting for baby in traditional Ao Dai, Year of the Snake, warm festive atmosphere',
            options: { keepFace: true, keepPose: true, matchLight: true }
        },
        {
            id: 'baby_tet_lucky',
            name: 'Lộc Đầu Năm',
            icon: '🧧',
            bgColor: 'linear-gradient(135deg, #c62828 0%, #ffab00 100%)',
            category: 'baby',
            description: 'Bé với bao lì xì, tiền vàng may mắn',
            prompt: 'Vietnamese New Year 2026 lucky money theme, red envelopes (li xi), gold coins, prosperity decorations, bright red and gold colors, cute and festive setting for baby photos, Year of the Snake symbols',
            options: { keepFace: true, keepPose: false, matchLight: true }
        },
        {
            id: 'baby_tet_snake',
            name: 'Năm Tỵ 2026',
            ...
            category: 'baby',
            description: 'Bé với linh vật rắn may mắn năm Tỵ',
            prompt: 'Year of the Snake 2026 themed background with cute cartoon snake mascot, Vietnamese Tet decorations, peach blossoms, lanterns, red and gold theme, adorable and cheerful atmosphere for baby photography',
            options: { keepFace: true, keepPose: true, matchLight: true }
        },
        {
            id: 'baby_tet_flower',
            name: 'Vườn Hoa Xuân',
            icon: '🌸',
            bgColor: 'linear-gradient(135deg, #f8bbd9 0%, #fff59d 100%)',
            category: 'baby',
            description: 'Bé trong vườn hoa đào, mai rực rỡ',
            prompt: 'Vietnamese Spring garden with blooming peach blossoms (hoa đào) and apricot flowers (hoa mai), traditional Tet atmosphere, soft natural sunlight, pink and yellow flowers, dreamy and peaceful setting for baby photos',
            options: { keepFace: true, keepPose: true, matchLight: false }
        },

        // === TẾT - GIA ĐÌNH ===
        {
            id: 'family_tet_living',
            name: 'Phòng Khách Tết',
            icon: '🏠',
            bgColor: 'linear-gradient(135deg, #8d6e63 0%, #ffcc80 100%)',
            category: 'family',
            description: 'Gia đình sum họp trong phòng khách ngày Tết',
            prompt: 'Vietnamese family living room decorated for Tet 2026, traditional New Year decorations, lucky red banners (câu đối), peach blossom tree, ancestral altar, warm golden lighting, cozy family gathering atmosphere, Year of the Snake',
            options: { keepFace: true, keepPose: true, matchLight: true }
        },
        {
            id: 'family_tet_outdoor',
            name: 'Đường Hoa Xuân',
            icon: '🎋',
            bgColor: 'linear-gradient(135deg, #66bb6a 0%, #ffeb3b 100%)',
            category: 'family',
            description: 'Gia đình đi dạo đường hoa Tết',
            prompt: 'Vietnamese Tet flower street festival 2026, colorful flower displays, traditional lanterns, festive crowd atmosphere, spring sunshine, joyful celebration, perfect for family group photos during Lunar New Year',
            options: { keepFace: true, keepPose: false, matchLight: true }
        },
        {
            id: 'family_tet_temple',
            name: 'Chùa Đầu Năm',
            icon: '🏯',
            bgColor: 'linear-gradient(135deg, #795548 0%, #ff8a65 100%)',
            category: 'family',
            description: 'Gia đình đi lễ chùa cầu may',
            prompt: 'Vietnamese Buddhist temple during Tet 2026, traditional architecture, incense smoke, red lanterns, peaceful spiritual atmosphere, morning light, family praying for good fortune, Year of the Snake blessings',
            options: { keepFace: true, keepPose: true, matchLight: true }
        },
        {
            id: 'family_tet_dinner',
            name: 'Mâm Cỗ Tết',
            icon: '🍲',
            bgColor: 'linear-gradient(135deg, #ff7043 0%, #ffe082 100%)',
            category: 'family',
            description: 'Gia đình quây quần bên mâm cỗ ngày Tết',
            prompt: 'Vietnamese Tet family dinner setting 2026, traditional feast table with banh chung, candied fruits, lucky red decorations, warm indoor lighting, multi-generation family gathering, Year of the Snake celebration',
            options: { keepFace: true, keepPose: true, matchLight: true }
        },
        {
            id: 'family_tet_tradition',
            name: 'Giao Thừa',
            icon: '🎊',
            bgColor: 'linear-gradient(135deg, #1a237e 0%, #7c4dff 100%)',
            category: 'family',
            description: 'Gia đình đón giao thừa với pháo hoa',
            prompt: 'Vietnamese New Year Eve celebration 2026, spectacular fireworks display, midnight celebration, family watching fireworks together, festive atmosphere, red and gold decorations, Year of the Snake transition moment',
            options: { keepFace: true, keepPose: false, matchLight: true }
        },
        {
            id: 'family_tet_studio',
            name: 'Studio Tết',
            icon: '📷',
            bgColor: 'linear-gradient(135deg, #455a64 0%, #b71c1c 100%)',
            category: 'family',
            description: 'Chụp ảnh gia đình trong studio trang trí Tết',
            prompt: 'Professional photography studio decorated for Vietnamese Tet 2026, elegant red and gold backdrop, soft studio lighting, traditional Tet props, peach blossoms, formal family portrait setting, Year of the Snake theme',
            options: { keepFace: true, keepPose: true, matchLight: true }
        }
    ];
    ...
    async function handleTemplateComposite() {
        ...
        var cleanKey = key.trim();
        setLoading(true);
        addLog("=== TEMPLATE COMPOSITE ===");

        try {
            setStatus("Ghép template với AI...");

            // Build prompt for template
            var template = TEMPLATES.find(function (t) { return t.id === selectedTemplate; });
            if (!template) { throw new Error("Template không tìm thấy"); }

            // Use preset options from template
            var templateOptions = template.options || { keepFace: true, keepPose: true, matchLight: true };

            var basePrompt = "Generate a background image based on this description: " + template.prompt + ". Then composite the provided subject(s) onto this generated background.";
            var optionPrompts = [];

            // Apply preset options from template
            if (templateOptions.keepFace) {
                optionPrompts.push("CRITICAL: Keep all faces exactly the same, do not modify facial features, expression, or identity.");
            }
            if (templateOptions.keepPose) {
                optionPrompts.push("Maintain the exact same body poses and positioning of all subjects.");
            }
            ...
            optionPrompts.push("Make it look photorealistic and natural.");
            optionPrompts.push("Output high quality 4K image.");

            var prompt = basePrompt + " " + optionPrompts.join(" ");
            addLog("Template: " + template.name);
            addLog("Options: Face=" + templateOptions.keepFace + ", Pose=" + templateOptions.keepPose + ", Light=" + templateOptions.matchLight);
            addLog("Prompt: " + prompt.substring(0, 80) + "...");
            ...
            if (imagePart) {
                addLog("Template composite done! Length: " + imagePart.data.length);
                await placeBase64("data:" + (imagePart.mimeType || "image/png") + ";base64," + imagePart.data);
                addLog("DONE!");
                setStatus("Xong!");
                addCost('image', 1);
                ...
                throw new Error("No template composite returned");
            }
        } catch (e) {
            addLog("Error: " + e.message);
            setStatus("Lỗi");
